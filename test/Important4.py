...
if __name__ == "__main__":
    input_file_path_posix =Path("pdf\\Complex\\ComplexPDF.pdf") #Path(sys.argv[1])
    ...
    meta_data = get_meta_data("pdf\\Complex\\ComplexPDF.pdf")
    page_layouts = get_page_layouts("pdf\\Complex\\ComplexPDF.pdf", meta_data["total_pages"])
                ...
                if isinstance(image_info, list):
                    for image_info_index, candidate in enumerate(image_info):
                        try:
                            candidate["image_save_path"] = output_base_path.joinpath(
                                f"Page - {page_layout_index + 1} - {figure_index + 1} - {candidate['mode']}.{candidate['type']}")
                            pprint(image_info)
                    ...
                    if image_info["type"] is not False:
                        image_info["image_save_path"] = output_base_path.joinpath(
                            f"Page - {page_layout_index + 1} - {figure_index + 1}.{image_info['type']}")
                        pprint(image_info)
